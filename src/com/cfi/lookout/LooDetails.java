...
public class LooDetails extends android.support.v4.app.FragmentActivity
{
	...
	private Loo				looInfo			= null;
	private boolean			mResolved		= false;
	
	public static int sLastVisitedLoo = 1;
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		mContext = this;
		mActivity = this;
		setContentView(R.layout.loodetails);
		utils.AddAdView(this);

		mReference = this.getIntent().getExtras().getString("reference");
		looInfo = publicLoosUtils.getLooInfo(mReference);

		mDistance = this.getIntent().getExtras().getDouble("distance");
		...
		try {
			mResolved = this.getIntent().getExtras().getBoolean("resolved");
		}catch(Exception e){}

		mInitialized = false;
		PopulateUIViewsFromStaticData();
	}
	...
	protected void onResume()
	{
		...
		if( mInitialized == false )
		{
			...
			TextView openingHrs = (TextView) findViewById(R.id.opening_hours_string);
			openingHrs.setText(looInfo.timing);
			
			TextView looType = (TextView) findViewById(R.id.loo_type_string);
			looType.setText(looInfo.type);
			
			TextView urinalsCountTextView = (TextView) findViewById(R.id.urinals_count_string);
			urinalsCountTextView.setText("" + looInfo.urinal_count);
			
			TextView paidTextView = (TextView) findViewById(R.id.free_paid_string);
			if( looInfo.paid == false )
				paidTextView.setText("Free");
			else
				paidTextView.setText("Paid");
			
			TextView handicapTextView = (TextView) findViewById(R.id.support_for_handicap_string);
			if( looInfo.handicap_support )
				handicapTextView.setText("Yes");
			else
				handicapTextView.setText("No");
			
			mHandler.dispatchMessage(mHandler.obtainMessage(MESSAGE_PLACE_DETAILS_FETCH));
			
			if( mResolved )
			{
				TextView lastServicesView = (TextView) findViewById(R.id.last_serviced);
				lastServicesView.setText("Few moments ago");
			}
						...
						public void run()
						{
							...
							try
							{
								safeUrl = "https://maps.googleapis.com/maps/api/place/details/json?reference=" + mReference
										+ "&sensor=false&key=AIzaSyCljUfAeaCF--PdZ-qYsgEB7YzBLmyuw2E";
						...
						public void run()
						{
							...
							try
							{
								safeUrl = "https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=" + mPhotoReference
										+ "&sensor=false&key=AIzaSyA998X5AFH7cDruAov4YohN46jJDB7UmaI";
	...
	private void PopulateUIViewsFromStaticData()
	{
		if( mName != null )
		{
		...
		if( mRating >= 0.1 )
		{
			...
			else if( mRating < 1.25 )
				indexForRatingBitmap = 1;
			else if( mRating < 1.75 )
				indexForRatingBitmap = 2;
			else if( mRating < 2.25 )
				indexForRatingBitmap = 3;
			else if( mRating < 2.75 )
				indexForRatingBitmap = 4;
			else if( mRating < 3.25 )
				indexForRatingBitmap = 5;
			else if( mRating < 3.75 )
				indexForRatingBitmap = 6;
			else if( mRating < 4.25 )
				indexForRatingBitmap = 7;
			else if( mRating < 4.75 )
		...
		phone_number.setText("9984012345");
    ...
    {
    	sLastVisitedLoo = Integer.parseInt(mReference);
    	new Thread(new Runnable()
		{
		        public void run()
		        {
		        	...
		        	LooDetails.this.runOnUiThread(new Runnable()
		  			{
                         @Override
                         public void run()
                         {
                        	 LooDetails.this.startService(new Intent(LooDetails.this, OnAlarmService.class));
                         }
		  			});
		        }
		}).start();
	...
	{
		if (v.getTag().equals("report_an_issue"))
		{
